...
//    private static final String INVENTORY_SERVICE_URL = "http://inventory-service";
    ...
    public Cart getCartByDealerId(String dealerId) throws CartNotFoundException {
		return cartRepository.findByDealerId(dealerId).orElseThrow(()-> new CartNotFoundException("Cart not found for dealer: " + dealerId));
	}

    public Cart addToCart(String dealerId, String cropId, int quantity) {

        Cart cart = cartRepository.findByDealerId(dealerId).get();
        if (cart == null) {
            cart = new Cart();
    ...
    public Cart updateCartItemQuantity(String dealerId, String cropId, int quantity) throws CartNotFoundException, InvalidCropException{
